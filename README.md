...
| **V7** | Is there a **falsifiable real-data signature** of a discrete (hypercubic-lattice) spacetime substrate? (UHECR axis-anisotropy test) | **V7.1 — V7_ENGINE_VALID_READY_FOR_REAL_DATA: control-gated statistic with MEASURED type-I calibration (median p=0.475, FPR@.05=0.053 → look-elsewhere absorbed, no detector bias) and a sensitivity floor (50% power at ~6% on-axis fraction); positive z=+117.6 fires, isotropic null silent, ingestion self-tested. NOT yet run on observational data (null expected); bounds a lattice substrate, cannot prove a simulator** | `report_v7.md`, `v7_lattice.py`, `v7_artifacts/v7_controls.json` |
