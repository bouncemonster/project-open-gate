"""
Proof of Simulation V3 — dedicated evidence store (history_v3.sqlite3).

V3 is isolated from V1/V2: its own file name, SQLite application_id and schema
version. Legacy or incompatible databases are refused read-only, never migrated.
Every reported number is persisted here so report generation can verify it.
"""
import json
import sqlite3
import time

V3_MODEL = "V3_ARITHMETIC_ISOLATION"
...
class V3Store:
    ...
    def __init__(self, path, protocol_hash, source_hash, binary_hash):
        ...
        if path.is_dir():
            path /= "history_v3.sqlite3"
        resolved = path.resolve()
        if (path.name.lower() != "history_v3.sqlite3"
                or resolved.name.lower() != "history_v3.sqlite3"
                or "archives" in (p.lower() for p in resolved.parts)):
            raise ValueError("V3 requires dedicated history_v3.sqlite3 outside archives")
        self.path = str(path)
    ...
    def _check_identity(self, conn):
        ...
        if (identity, version) != (self.APPLICATION_ID, self.SCHEMA_VERSION):
            raise ValueError("Refusing legacy or incompatible database; no migration")

    def integrity(self):
        status = self.conn.execute("PRAGMA integrity_check").fetchone()[0]
        fk = self.conn.execute("PRAGMA foreign_key_check").fetchall()
        if status != "ok" or fk:
            raise RuntimeError(f"V3 database integrity failure: {status} {fk}")
        return status
    ...
    def create_run(self, run_id, config):
        ...
        with self.conn:
            ...
            if exists:
                ...
                if row[0] != serialized:
                    raise ValueError(f"Incompatible V3 resume: config differs for {run_id}")
            else:
                self.conn.execute(
    ...
    def commit_step(self, run_id, step, evaluations, state):
        with self.conn:
            self.conn.execute(
                "INSERT OR REPLACE INTO checkpoints(run_id,step,state_json) VALUES(?,?,?)",
                (run_id, step, _json(state)))
            for cand in evaluations:
                result = cand.get("result", {})
                metrics = result.get("metrics", result)
                self.conn.execute(
                    "INSERT OR REPLACE INTO evaluations(run_id,step,candidate_id,role,target,"
                    "width,height,model_id,driver,cr,ci,alpha,degree,channel,max_iter,seed,"
                    "driver_hash,score,status,params_json,result_json) "
                    "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                    (run_id, step, int(cand["candidate_id"]),
                     cand.get("role", "PRIMARY"), cand.get("target", "TARGET_A"),
                     int(cand.get("width", 60)), int(cand.get("height", 30)),
                     cand.get("model_id", V3_MODEL), cand["driver"],
                     float(cand["cr"]), float(cand["ci"]), float(cand.get("alpha", 1.0)),
                     int(cand.get("degree", 2)), int(cand.get("channel", 0)),
                     int(cand.get("max_iter", 200)), int(cand.get("seed", 7331)),
                     cand.get("driver_hash", ""),
                     metrics.get("score"), cand.get("status", "OK"),
    ...
    def run_rows(self, run_id):
        with self.conn:
            rows = self.conn.execute(
                "SELECT * FROM evaluations WHERE run_id=? ORDER BY step, candidate_id",
                (run_id,)).fetchall()
    ...
    def best_of(self, run_id):
        with self.conn:
            row = self.conn.execute(
                "SELECT * FROM evaluations WHERE run_id=? AND status='OK' AND score IS NOT NULL "
                "ORDER BY score DESC LIMIT 1", (run_id,)).fetchone()
        return dict(row) if row else None

    def count_by_role(self):
        with self.conn:
            rows = self.conn.execute(
                "SELECT role, COUNT(*) c FROM evaluations GROUP BY role").fetchall()
        return {r["role"]: r["c"] for r in rows}

    def total_evaluations(self):
        with self.conn:
            return self.conn.execute("SELECT COUNT(*) FROM evaluations").fetchone()[0]

    # ---- generic phase results ----------------------------------------
    def record(self, phase, key, value, score=None):
        self.integrity()
        with self.conn:
            self.conn.execute(
                "INSERT OR REPLACE INTO results(phase,key,score,value_json,timestamp) "
                "VALUES(?,?,?,?,?)", (phase, key, score, _json(value), _now()))
    ...
    def all_results(self, phase=None):
        with self.conn:
            ...
            for r in self.conn.execute(sql, args).fetchall():
                out[f"{r['phase']}:{r['key']}"] = {
                    "phase": r["phase"], "key": r["key"], "score": r["score"],
    ...
    def set_counter(self, name, value):
        with self.conn:
            self.conn.execute(
                "INSERT OR REPLACE INTO counters(name,value) VALUES(?,?)", (name, value))
    ...
    def counters(self):
        with self.conn:
            return {r["name"]: r["value"]
                    for r in self.conn.execute("SELECT name,value FROM counters").fetchall()}
