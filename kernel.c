/*
 * PROOF OF SIMULATION — C Numerical Kernel V2
 * Non-autonomous Julia set iteration with prime-arithmetic forcing.
 * V2 corrections: variable grid, heap allocation, drivers V9-V13/S1/S2,
 *   one-pass trace, strict input validation, JSON evaluate, target-field,
 *   score-fields, driver-values, Nyquist fix, log(degree) smoothing.
 * Compile: gcc -O3 -std=c11 -Wall -Wextra -Wpedantic kernel.c -lm -o kernel_v2
 */
...
static int self_test(void) {
    ...
    if (!T || !F || !d1 || !d2 || !drv) { printf("FAIL: self_test OOM\n"); pass = 0; }
        ...
        iterate_julia(-0.7, 0.27015, drv, 1.0, 2, 0, 200, 60, 30, d1, NULL, NULL, NULL,NULL,NULL,NULL,NULL,NULL, NULL, NULL, 0);
        iterate_julia(-0.7, 0.27015, drv, 1.0, 2, 0, 200, 60, 30, d2, NULL, NULL, NULL,NULL,NULL,NULL,NULL,NULL, NULL, NULL, 0);
        double max_diff = 0;
...
int main(int argc, char **argv) {
    ...
    if (!strcmp(argv[1], "--batch")) {
        ...
        process_batch(W, H, tgt_id, pn, pm, tgt_file, window, offset);
        return 0;
    }
    ...
    if (!strcmp(argv[1], "--evaluate") || !strcmp(argv[1], "--render") || !strcmp(argv[1], "--trace")) {
        ...
        if (!strcmp(argv[1], "--evaluate"))
            run_evaluate(cr, ci, drv, alpha, degree, channel, max_iter, seed, W, H, tgt_id, pn, pm, tgt_file, window, offset);
            ...
            run_trace(cr, ci, drv, alpha, degree, channel, max_iter, seed, W, H, window, offset);
        return 0;
    }
