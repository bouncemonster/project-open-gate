...
def main():
    ...
    parser.add_argument("command", choices=[
        ...
        "v2-auto", "v3-auto", "v3-resume", "v4-auto", "v4-resume"
    ])
    ...
    parser.add_argument("--mode", type=str, default=None,
                        help="Pipeline mode: deep-v3, deep-v4, v5.2-deep, v5.3-deep")
    parser.add_argument("--null-runs", type=int, default=None)
    parser.add_argument("--phases", type=str, default=None,
                        help="V3: comma-separated phase list")
    parser.add_argument("--resume-from", type=str, default=None,
                        help="V3: resume from specific phase")
    ...
    if args.command == "init":
        ...
        loop.cmd_benchmark()
    elif args.command == "v4-auto" or (args.command == "auto" and args.mode == "deep-v4"):
        from v4_pipeline import V4Pipeline
        p = V4Pipeline(time_budget=args.time_budget, quiet=(args.ui == "quiet"),
                       null_runs=args.null_runs)
        phases = args.phases.split(",") if args.phases else None
        ok = p.run(phases=phases)
        sys.exit(0 if ok else 1)
    elif args.command == "v4-resume" or (args.command == "resume" and args.mode == "deep-v4"):
        from v4_pipeline import V4Pipeline
        p = V4Pipeline(time_budget=args.time_budget, quiet=(args.ui == "quiet"),
                       null_runs=args.null_runs)
        phases = args.phases.split(",") if args.phases else None
        ok = p.run(phases=phases, resume_from=args.resume_from)
        sys.exit(0 if ok else 1)
    elif args.command == "v3-auto" or (args.command == "auto" and args.mode == "deep-v3"):
        from v3_pipeline import V3Pipeline
        p = V3Pipeline(time_budget=args.time_budget, quiet=(args.ui == "quiet"),
                       null_runs=args.null_runs)
        phases = args.phases.split(",") if args.phases else None
        ok = p.run(phases=phases)
        sys.exit(0 if ok else 1)
    elif args.command == "auto" and args.mode == "v5.3-deep":
        from v5_3_pipeline import run_deep
        ok = run_deep(time_budget=args.time_budget, resume=False)
        sys.exit(0 if ok else 1)
    elif args.command == "resume" and args.mode == "v5.3-deep":
        from v5_3_pipeline import run_deep
        ok = run_deep(time_budget=args.time_budget, resume=True)
        sys.exit(0 if ok else 1)
    elif args.command == "auto" and args.mode == "v5.2-deep":
        from v5_2_pipeline import run_deep
        ok = run_deep(time_budget=args.time_budget, resume=False)
        sys.exit(0 if ok else 1)
    elif args.command == "resume" and args.mode == "v5.2-deep":
        from v5_2_pipeline import run_deep
        ok = run_deep(time_budget=args.time_budget, resume=True)
        sys.exit(0 if ok else 1)
    elif args.command == "v3-resume" or (args.command == "resume" and args.mode == "deep-v3"):
        from v3_pipeline import V3Pipeline
        p = V3Pipeline(time_budget=args.time_budget, quiet=(args.ui == "quiet"),
                       null_runs=args.null_runs)
        phases = args.phases.split(",") if args.phases else None
