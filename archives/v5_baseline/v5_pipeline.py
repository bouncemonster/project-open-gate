...
class V5Pipeline:
    ...
    def phase_generate_worlds(self):
        ...
        for world_type, config in self.protocol["world_types"].items():
            ...
            # Generate paired worlds for continuous types
            if config["category"] == "continuous":
                self._log(f"  Generating paired worlds for {world_type}...")
                for seed_offset in range(min(5, seeds_per_world)):
                    seed = self.base_seed + seed_offset * 1000 + config["id"] * 100
                    pairs = generate_paired_worlds(world_type, width, height, seed)
                    
                    for pair_type, pair_field in pairs.items():
                        if pair_type == world_type:
                            continue
                        
                        pair_config = self.protocol["world_types"].get(pair_type)
                        if not pair_config:
                            continue
                        
                        pair_hash = hashlib.sha256(str(pair_field[:100]).encode()).hexdigest()[:16]
                        pair_id = self.db.insert_world(
                            pair_type, seed, width, height, "O1_FULL_PRECISION", pair_hash
                        )
                        self.db.assign_split(pair_id, pair_config["split"])
                        ...
                        self.worlds[pair_id] = {
                            ...
                            "width": width,
                            "height": height,
                            "split": pair_config["split"],
                            "category": pair_config["category"]
                        }
                        
                        # Record pairing
                        base_id = self.db.conn.execute(
                            "SELECT id FROM worlds WHERE world_type=? AND seed=?",
                            (world_type, seed)
                        ).fetchone()
                        if base_id:
                            self.db.insert_paired_world(base_id["id"], pair_id, "precision_pair")
    ...
    def phase_blind_holdout(self, detector_id: int):
        ...
        for cls in sorted(class_total.keys()):
            ...
            self._log(f"  {cls}: {cls_acc:.4f} ({class_correct[cls]}/{class_total[cls]})")
